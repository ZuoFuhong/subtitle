...
namespace wav {
    ...
    public:
        ...
        bool open_file(const std::string& filename) {
            ...
            if (header.fmt_size < 16) {
                std::cout << "WaveData: expect PCM format data to have fmt chunk of at least size 16." << std::endl;
                return false;
            } else if (header.fmt_size > 16) {
                int offset = static_cast<int>(44 - 8 + header.fmt_size - 16);
            ...
            int num_data = static_cast<int>(header.data_size / (bits_per_sample_ / 8));
            data_ = new float[num_data];
            num_samples_ = num_data / num_channel_;

            std::cout << "sample_rate: " << sample_rate_ << " num_channel: " << num_channel_ << " bits_per_sample: " << bits_per_sample_ << std::endl;
            switch (bits_per_sample_) {
                ...
                case 32:
                {
                    if (header.format == 1) { // S32
                        ...
                        std::cout << "unsupported quantization bits" << std::endl;
                        return false;
                    }
                    break;
                } default: {
        ...
        [[nodiscard]] int num_channel() const {
            return num_channel_;
        }
    ...
    class WavWriter {
    public:
        WavWriter(const float* data, int num_samples, int num_channel, int sample_rate, int bits_per_sample)
                : data_(data),
            ...
            for (int i = 0; i < num_samples_; ++i) {
                for (int j = 0; j < num_channel_; ++j) {
                    switch (bits_per_sample_) {
                        ...
                        case 16:
                        {
                            auto sample = static_cast<int16_t>(data_[i * num_channel_ + j]);
                            fwrite(&sample, 1, sizeof(sample), fp);
                            break;
                        }
                        case 32:
